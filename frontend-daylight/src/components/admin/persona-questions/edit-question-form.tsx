...
import { AdminPersonaQuestion, UpdatePersonaQuestionPayload } from '@/types/admin-persona-question.types';
...
export function EditQuestionForm({ question }: EditQuestionFormProps) {
  ...
  const { updateQuestion } = useAdminPersonaQuestionMutations();

  const { register, handleSubmit, formState: { errors }, control, watch, setValue, reset } = useForm<UpdatePersonaQuestionPayload>({
    defaultValues: {
      ...
      options: question.options.map(opt => ({
        ...
        text: opt.text,
        traitImpacts: typeof opt.traitImpacts === 'string' 
          ? opt.traitImpacts 
          : JSON.stringify(opt.traitImpacts, null, 2),
      })),
    },
  });
  ...
  }, [updateQuestion.isSuccess, router]);

  const onSubmit = (data: UpdatePersonaQuestionPayload) => {
    ...
    const processedData = {
      ...
      options: data.options?.map(option => {
        ...
        if (typeof traitImpacts === 'string') {
          try {
            traitImpacts = JSON.parse(traitImpacts);
          } catch (e) {
