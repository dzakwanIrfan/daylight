...
export function SubscriptionUpsellModal({
  ...
  const [selectedOption, setSelectedOption] = useState<'onetime' | 'subscription'>('subscription');
  ...
  const savingsPerMonth = monthlyPlan ? costPerMonthOneTime - monthlyPlan.price : 0;
  const savingsPercentage = monthlyPlan 
        ...
        <div className="p-6 space-y-6">
          ...
          <DialogHeader className="space-y-2">
            ...
            <DialogDescription className="text-sm text-gray-600 leading-relaxed">
              <span className="font-semibold text-gray-900">{eventTitle}</span> costs{' '}
              <span className="font-semibold text-gray-900">{formatCurrency(eventPrice, currency)}</span>.
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4 h-12">
                <div>
                  <h3 className="font-bold text-lg text-gray-900 mb-0.5">Single Purchase</h3>
                  <p className="text-xs text-gray-600">Pay once for this event</p>
                </div>
                <div className={cn(
              ...
              <div className="pt-4 border-t border-gray-200 space-y-3 min-h-[140px]">
                <div className="flex items-start gap-2.5">
                  ...
                  <span className="text-sm text-gray-700">Access to this event</span>
                </div>
                <div className="flex items-start gap-2.5">
                  <X className="w-4 h-4 text-gray-400 shrink-0 mt-0.5" />
                  <span className="text-sm text-gray-500">Limited to single event</span>
                </div>
                <div className="flex items-start gap-2.5">
                  <X className="w-4 h-4 text-gray-400 shrink-0 mt-0.5" />
                  <span className="text-sm text-gray-500">No future access</span>
                </div>
              </div>
            </button>
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4 h-12 mt-2">
                <div>
                  <h3 className="font-bold text-lg text-gray-900 mb-0.5">Subscription</h3>
                  <p className="text-xs text-gray-600">Unlimited event access</p>
                </div>
                <div className={cn(
                  "w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ml-2",
                  selectedOption === 'subscription' 
                ...
                <div className="mb-6 h-16 flex flex-col justify-center">
                  <div className="flex flex-col">
                    <span className="text-3xl font-bold text-brand">
                      {formatCurrency(monthlyPlan.price, currency)}
                    </span>
              ...
              <div className="pt-4 border-t border-orange-200 space-y-3 min-h-[140px]">
                <div className="flex items-start gap-2.5">
                  ...
                  <span className="text-sm text-gray-900 font-medium">Unlimited events</span>
                </div>
                <div className="flex items-start gap-2.5">
                  <Check className="w-4 h-4 text-brand shrink-0 mt-0.5" />
                  <span className="text-sm text-gray-900 font-medium">Exclusive community</span>
                </div>
                <div className="flex items-start gap-2.5">
                  <Check className="w-4 h-4 text-brand shrink-0 mt-0.5" />
                  <span className="text-sm text-gray-900 font-medium">VIP access</span>
                </div>
              </div>
            </button>
          </div>
          ...
          {/* Savings Info - CLEAN */}
          {selectedOption === 'subscription' && monthlyPlan && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm text-green-800">
                <div className="flex items-center justify-between">
                  <span>Attending {eventsPerMonth} events/month:</span>
                  <span className="font-semibold">{formatCurrency(costPerMonthOneTime, currency)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>With subscription:</span>
                  <span className="font-semibold">{formatCurrency(monthlyPlan.price, currency)}</span>
                </div>
                <div className="flex items-center justify-between pt-2 border-t border-green-300">
                  ...
                  <span className="font-bold text-green-600">{formatCurrency(savingsPerMonth, currency)}</span>
                </div>
              </div>
            </div>
          )}
